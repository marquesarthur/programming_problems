...
class Solution(object):
    ...
    # def __get_min_node(self, dist, visited):
    #
    #     _min = sys.maxint
    #     result = -1
    #     for node, dist in enumerate(dist):
    #         if dist < _min and node not in visited:
    #             result = node
    #             _min = dist
    #
    #     return result
    #
    #
    # def __update_dist_of(self, v, edges, dist, visited):
    #     for i, e in enumerate(edges):
    #         if i not in visited and e > 0:
    #             if dist[v] + e < dist[i]:
    #                 dist[i] = dist[v] + e
    #
    #
    #
    # def djikstra(self, start, end, graph):
    #     n = len(graph)
    #     dist = [sys.maxint for _ in xrange(n)]
    #     dist[start] = 0
    #
    #     visited = set()
    #
    #     while len(visited) != n:
    #         v = self.__get_min_node(dist, visited)
    #         if v == -1:
    #             break
    #         visited.add(v)
    #         self.__update_dist_of(v, graph[v], dist, visited)
    #
    #     return dist[end]
    ...
    def bfs(self, start, end, graph):
        ...
        while queue:
            ...
            for next, edge in enumerate(graph[current]):
                if edge > 0 and next not in visited:
                    queue.append((next, hops + 1))
    ...
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        graph = [[0 for _ in xrange(len(wordList))] for _ in xrange(len(wordList))]
        ...
        for i in xrange(len(graph)):
            for j in xrange(len(graph)):
                ...
                diff = sum([0 if c1 == c2 else 1 for c1, c2 in zip(a, b)])
                if diff == 1:
                    graph[i][j] = diff
